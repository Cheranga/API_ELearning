...
namespace Api_ELearning.DataAccess.DataContext
{
    internal class DataSeeder
    {
        ...
        private static readonly string[] tutorNames =
        {
        ...
        public DataSeeder(LearningContext ctx)
        {
            _ctx = ctx;
        }
        ...
        public void Seed()
        {
            ...
            try
            {
                ...
                for (var i = 0; i < tutorNames.Length; i++)
                {
                ...
                for (var i = 0; i < studentNames.Length; i++)
                {
                    ...
                    var student = new Student
                    {
                        ...
                        EnrolledDate = DateTime.UtcNow.AddDays(-new Random().Next(365, 730))
                    };

                    _ctx.Students.Add(student);

                    var maxCoursesId = _ctx.Courses.Max(c => c.Id);

                    //To enroll in 4 courses randomly
                    for (var z = 0; z < 4; z++)
                    {
                        var randomCourseId = new Random().Next(1, maxCoursesId);

                        var enrollment = new Enrollment
                        {
                            Student = student,
                            Course = _ctx.Courses.Where(c => c.Id == randomCourseId).Single(),
                            EnrolledDate = DateTime.UtcNow.AddDays(-new Random().Next(10, 30))
                        };
        ...
        {
            var _rng = new Random((int) DateTime.Now.Ticks);
